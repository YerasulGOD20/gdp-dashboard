...
def get_predictions_data():
    ...
    st.error("Не удалось найти файл с данными. Поместите файл predictions.xlsx или Sheet1.csv в папку data/")
    return pd.DataFrame()
...
Анализ данных предсказаний из нашего проекта по data mining.
'''
...
with col1:
    st.metric(label="Всего записей", value=f"{len(predictions_df):,}")

with col2:
    st.metric(label="Количество колонок", value=len(predictions_df.columns))

with col3:
    numeric_cols = predictions_df.select_dtypes(include=['number']).columns
    st.metric(label="Числовых колонок", value=len(numeric_cols))
...
if len(numeric_cols) > 0:
    ...
    ''
    selected_column = st.selectbox('Выберите колонку для визуализации:', numeric_cols)

    if selected_column:
        chart_data = predictions_df[[selected_column]].copy().reset_index(drop=True)
